...
# Model definition
class SimpleModel:
    def __init__(self, input_dim = 200, train_size = 100, hid_dim = 100, layers = 3, mc_iter = 10, step_size = 0.01, sigma = 1):
        ...
        for l in range(self.nlayer - 1, 0, -1):
            for j in range(self.hid_dim):
                ...
                for t in range(self.mc_iter):
                    ...
                    w = np.zeros(n)
                    for k in range(n):
                        w[k] = pg.pgdraw(1, np.dot(self.layers[l-1].h[k,:], curr))
                    # Draw posterior beta
    ...
    def flipAndReject(self, x, y, i):
        accept = 0
        for l in range(0, self.nlayer):
            if l == 0:
                curr_probs = self.layers[0].det_forward(x, i)
            else:
                curr_probs = self.layers[l].det_forward(self.layers[l-1].h[i], i)
            for j in range(self.hid_dim):
                prop_h = self.layers[l].h[i]
                prop_h[j] = 1 - prop_h[j]
                next_probs_prop = self.layers[l+1].det_forward(prop_h, i)
                next_probs_curr = self.layers[l+1].det_forward(self.layers[l].h[i], i)
                logalpha = logbern(prop_h[j], curr_probs[j]) - logbern(1 - prop_h[j], curr_probs[j])
                if l == self.nlayer-1:
                    logalpha += logbern(y, next_probs_prop)
                    logalpha -= logbern(y, next_probs_curr)
                else:
                    for k in range(self.hid_dim):
                        logalpha += logbern(self.layers[l+1].h[i,k], next_probs_prop[k])
                        logalpha -= logbern(self.layers[l+1].h[i,k], next_probs_curr[k])
                if np.log(np.random.uniform()) < logalpha:
                    self.layers[l].h[i,j] = 1 - self.layers[l].h[i,j]
                    accept += 1
        return accept
...
model = SimpleModel(input_dim, train_size, hid_dim, layers, mc_iter) # Model init
...
for t in range(epochs):
    ...
    for i in range(train_size):
        ...
        #a += model.permAndReject(X_train[i], y_train[i], i)
        #a += model.backpropAndReject(X_train[i], y_train[i], i) # h update for each obs
        a += model.flipAndReject(X_train[i], y_train[i], i)
    model.logisticAndReject(X_train, y_train) # W update using all obs
    print "H Acceptance Proportion: " + str(float(a)/(train_size*layers*hid_dim))
